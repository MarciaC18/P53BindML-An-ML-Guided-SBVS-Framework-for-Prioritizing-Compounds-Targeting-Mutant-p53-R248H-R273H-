# =========================================================
# FULL SCRIPT: PLEC + CV + FINAL TRAIN + TEST (XGB)
...
# the outer cross-validation, and (2) XGBClassifier's random_state. Unlike an
# RBF-kernel SVM, XGBoost is not fully deterministic even with a fixed seed:
# with subsample=1.0 and colsample_bytree=1.0 (library defaults, not set
# explicitly here), there is no row/column subsampling randomness, but
# random_state still affects tie-breaking during split search, and
# multi-threaded execution (n_jobs=40) can introduce minor floating-point
# non-determinism in histogram accumulation across runs. The largest source
# of run-to-run variability, however, remains the outer CV fold split.
#
...
# -----------------------------
logging.basicConfig(
    filename="plec_pipeline_XGB.log",
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s"
)
logging.info("===== START PIPELINE: PLEC + XGB 10 RUNS =====")
...
def process_dataset(folder, dataset_name="TRAIN", num_cores=16):
    ...
    # Ligands
    lig_files = glob.glob(str(Path(folder) / "ligands" / "*.mol2"))
    ligands, names = [], []
    for f in lig_files:
    ...
    if df.isna().sum().sum() != 0:
        raise RuntimeError(f"[{dataset_name}] NaNs detected after merge")

    # Receptors
    rec_files = glob.glob(str(Path(folder) / "receptor" / "*.mol2"))
    receptors = {Path(f).stem: next(oddt.toolkit.readfile("mol2", f)) for f in rec_files}
    ...
    if not is_binary:
        logging.info(f"[{dataset_name}] PLEC features are count-based (max value = "
                      f"{unique_vals.max()}). No action needed for XGBoost, since "
...
for run in range(1, num_runs + 1):
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(train_features_array, y_train), 1):
        xgb_fold = XGBClassifier(
            n_jobs=40,
            eval_metric="logloss",
            random_state=run_seed
        )
        xgb_fold.fit(train_features_array[tr_idx], y_train[tr_idx])

        # ALWAYS verify which column of predict_proba corresponds to the
        # positive class — never assume column order.
        assert set(xgb_fold.classes_) == {0, 1}, f"Unexpected classes_ in fold {fold}: {xgb_fold.classes_}"
        idx_active_fold = list(xgb_fold.classes_).index(1)

        prob_val = xgb_fold.predict_proba(train_features_array[val_idx])[:, idx_active_fold]
        pred_val = (prob_val >= 0.5).astype(int)

        roc_auc_cv.append(roc_auc_score(y_train[val_idx], prob_val))
        p, r, _ = precision_recall_curve(y_train[val_idx], prob_val, pos_label=1)
        pr_auc_cv.append(auc_prc(r, p))
        f1_cv.append(f1_score(y_train[val_idx], pred_val))
        mcc_cv.append(matthews_corrcoef(y_train[val_idx], pred_val))
    ...
    cv_df.to_csv(f"{output_dir}/XGB_CV_run{run}_{timestamp}.csv", index=False)
    ...
    xgb_final.fit(train_features_array, y_train)
    ...
    assert set(xgb_final.classes_) == {0, 1}, f"Unexpected classes_ in run {run}: {xgb_final.classes_}"
    idx_active = list(xgb_final.classes_).index(1)
    idx_inactive = list(xgb_final.classes_).index(0)
    logging.info(f"[Run {run}] xgb_final.classes_={list(xgb_final.classes_)} -> "
                 f"idx_active={idx_active} idx_inactive={idx_inactive}")

    # TRAIN metrics
    train_prob = xgb_final.predict_proba(train_features_array)[:, idx_active]
    ...
    # ---------- TEST ----------
    test_prob = xgb_final.predict_proba(test_features_array)[:, idx_active]
    ...
    results_df = pd.DataFrame({
        ...
        "Inactive_Prob": xgb_final.predict_proba(test_features_array)[:, idx_inactive],
        "Predicted_Class": test_pred,
        "Real_Class": y_test
    })
    results_df.replace({1: POSITIVE_CLASS, 0: NEGATIVE_CLASS}, inplace=True)
    results_df.to_csv(f"{output_dir}/XGB_results_run{run}_{timestamp}.csv", index=False)
...
summary_df.to_csv(
    os.path.join(output_dir,
                 f"summary_10runs_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.csv"),
    index=False
)
...
final_model_file = os.path.join(model_dir, f"XGB_final_model_run{num_runs}.pkl")
dump(last_run_model, final_model_file)
print(f"✅ Final XGB model saved (run {num_runs}) in: {final_model_file}")
...
logging.info("===== END OF PIPELINE: PLEC + XGB 10 RUNS =====")
